...
## 2026-07-19: /babysit-prにレビュー完了の判定基準を追加

### 変更内容

- `/babysit-pr`(両テンプレート)に「レビュー完了の判定」節を追加した。再レビュー依頼後は次の2条件をポーリングし、両方を満たすまで完了と判定しない: (1) `requested_reviewers`の`users`からCopilotが消えている、(2) 依頼時刻より後に提出された本文非空のCopilotレビュー(総評)が存在する。
- 判定は本文長のみで行い、見出し文字列に依存しない(見出しはフォーマット変更・言語設定で変わりうるため)。
- 背景: Copilotは1回のレビューで複数のレビューレコードを生成し、本文が空のCOMMENTEDレビュー(インラインコメントの器)が総評より先に現れる。新規レコード1件の出現だけで完了と判定すると、中間レコードを「新規指摘なし」と誤読する問題があった。

### レビュー対応(経緯の記録。最終差分に変更なし)

- PR内で手順3cの継続コマンド行のインデントを一時的に3スペースへ変更したが、Copilotの指摘(サブリストの継続行として浅すぎる)に従い6スペースへ戻した(ユーザー確認済み)。往復したためPRの最終差分にインデント変更は含まれない。決定事項: 継続行インデントは6スペースを正とする。

### メタリポジトリで/babysit-pr等のコマンドを使えるようにする

- 要望: `/babysit-pr`をこのメタリポジトリ自体でも使えるようにする(コマンドなしで手順を代行するのは進捗が見えず不便)。
- 対応: `template_ja/.claude/commands/`の5コマンド(babysit-pr・merge-pr・commit・ship・consistency-check)をルート`.claude/commands/`へバイト同一でコピーし、`docs/tests/hook-tests.sh`にルートガードフックと同様の同一性チェックを追加した。AGENTS.mdの構成表も更新。
- 設計判断: コマンドは文書類だが、ルートコピーはtemplate_ja版と機械的に一致させて乖離を防ぐ(このリポジトリの作業言語は日本語のためtemplate_ja版を採用)。

### 記録

- 検証コマンド`sh docs/tests/hook-tests.sh`のフック挙動テスト6件がWindows(Git Bash)で失敗する既存問題をIssue #8として登録した(今回の変更とは無関係。テンプレート間同一性チェックはPASS)。

## 2026-07-20: grill-meスキルの追加と日本語版の作成
...
## 2026-08-12: GitHub CopilotとClaude Codeでスキルを共有
