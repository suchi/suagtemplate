...
AGENTS.mdや設定ファイルに書いても機械的には効かない・各ツールの仕様として知っておく必要がある注意点をまとめる。本ファイルはテンプレートには含まれない(このリポジトリの`docs/`にのみある)ため、テンプレートから作ったリポジトリで必要な項目はAGENTS.mdに転記するか、このファイルをコピーすること。
...
- claude.ai/code(Web)セッションにはgh CLIがない。GitHub操作はGitHub MCPツール経由になる。`/babysit-pr`は環境判別の指示を内蔵しており、WebセッションではGitHub MCP+PRイベント購読で同じループを回す。`/merge-pr`はgh前提なのでローカル用。
...
- 再レビュー依頼は`gh pr edit <番号> --add-reviewer @copilot`(gh 2.88.0以上)。WebセッションではGitHub MCPの`request_copilot_review`ツールを使う。REST API直叩き・PRのクローズ/リオープンでは再レビューはトリガーされない。
