...
| フック | イベント | 内容 |
| --- | --- | --- |
| `block-dangerous-git.sh` | PreToolUse(Bash) | main/masterへの直接push・force pushと`--no-verify`をdeny。force push(他ブランチ)・`reset --hard`・`clean -f`・`branch -D`・mainブランチ上でのcommitはユーザー確認(ask)にエスカレート |
